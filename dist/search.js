...
async function searchPlaces(e){e.preventDefault();const q=$('remote-query').value.trim();if(!q||searchBusy)return;
 ...
 $('search-message').textContent=searchResults.length?'点击结果，在地图上预览后加入。':'没有找到。试试“县市名 + 地点名”或其他名称；OSM 可能尚未收录。';
...
function previewPlace(p){if(!p)return;$('place-search-dialog').close();closeDetail();document.querySelector('.sidebar').classList.remove('open');$('list-toggle').setAttribute('aria-expanded','false');$('list-toggle').textContent='地点列表';if(previewMarker)map.removeLayer(previewMarker);
 ...
 previewMarker=L.marker([Number(p.lat),Number(p.lon)],{title:'搜索预览：'+title.textContent}).addTo(map).bindPopup(box,{maxWidth:290});map.setView([Number(p.lat),Number(p.lon)],13,{animate:false});previewMarker.openPopup();}
async function addPlace(p,button){const name=p.name||p.display_name.split(',')[0],lat=Number(p.lat),lon=Number(p.lon);const osmKey=`${p.osm_type}/${p.osm_id}`;
 ...
 const r={id,custom:true,osmKey,name,display_name:name,original_name:name,lat,lon,elevation:null,group:'我的新增地点',crs:'WGS84',url:['node','way','relation'].includes(p.osm_type)&&/^\d+$/.test(String(p.osm_id))?`https://www.openstreetmap.org/${p.osm_type}/${p.osm_id}`:'https://www.openstreetmap.org/',access:'自行添加的地点，车辆能否到达及开放情况尚未核实。',point:p.display_name,forecast:{time:[]},history:{time:[]},weather_note:'海拔与天气尚未补齐；如未显示，可点击下方重新查询。'};
