...
const worker=createWorker({'/index.html':{type:'text/html',body:btoa('<h1>EasyTrip</h1>')}},async(url,opts)=>{url=new URL(url);called++;assert.equal(url.origin,'https://restapi.amap.com');assert.equal(url.searchParams.get('key'),env.AMAP_JS_KEY);assert.equal(url.searchParams.get('jscode'),env.AMAP_SECURITY_CODE);assert.equal(opts.redirect,'manual');return new Response('{"status":"1","pois":[]}',{headers:{'content-type':'application/json','set-cookie':'private=1'}});});
